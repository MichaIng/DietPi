...
PRETTY=$NAME
ORGA='libsdl-org'
version=$(curl -sSf "${header[@]}" "https://api.github.com/repos/$ORGA/SDL/releases" | grep -Po '"name": *"\K2\.[^"]+(?=")' | head -1)
...
NAME='SDL2_image'
PRETTY=$NAME
version=$(curl -sSf "${header[@]}" "https://api.github.com/repos/$ORGA/SDL_image/releases" | grep -Po '"name": *"\K2\.[^"]+(?=")' | head -1)
[[ $version ]] || Error_Exit "No latest $PRETTY version found"
G_DIETPI-NOTIFY 2 "Building $PRETTY version \e[33m$version"
G_EXEC cd /tmp
...
NAME='SDL2_ttf'
PRETTY=$NAME
version=$(curl -sSf "${header[@]}" "https://api.github.com/repos/$ORGA/SDL_ttf/releases" | grep -Po '"name": *"\K2\.[^"]+(?=")' | head -1)
[[ $version ]] || Error_Exit "No latest $PRETTY version found"
G_DIETPI-NOTIFY 2 "Building $PRETTY version \e[33m$version"
G_EXEC cd /tmp
